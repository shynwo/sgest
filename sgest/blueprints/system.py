...
@bp.get("/system/integrations")
def system_integrations_page():
    user = _current_auth_user()
    if not user:
        return redirect(url_for("auth.login", next="/system/integrations"))
    smtp = _smtp_config()
...
@bp.post("/system/integrations/smtp")
def system_integrations_smtp_update():
    ...
    return redirect(url_for("system.system_integrations_page"))
@bp.post("/system/integrations/apis")
def system_integrations_apis_update():
    ...
    return redirect(url_for("system.system_integrations_page"))
@bp.get("/system/profile")
def system_profile_page():
    current_user = _current_auth_user()
    if not current_user:
        return redirect(url_for("auth.login", next="/system/profile"))
...
@bp.post("/system/profile/password")
def system_profile_password_update():
    ...
    if not pw_ok:
        ...
        return redirect(url_for("system.system_profile_page"))
...
def system_profile_avatar_update():
    ...
    return redirect(url_for("system.system_profile_page"))
@bp.post("/system/profile/avatar/reset")
def system_profile_avatar_reset():
    ...
    return redirect(url_for("system.system_profile_page"))
@bp.post("/system/profile/recovery-email")
def system_profile_recovery_email_update():
    ...
    return redirect(url_for("system.system_profile_page"))
@bp.post("/system/profile/users/add")
...
@bp.post("/system/backup-health-now")
def backup_health_now():
    try:
        ...
        return jsonify(ok=False, error=str(e)), 500
@bp.post("/system/restore-test-now")
...
@bp.get("/system/backup/<path:name>")
